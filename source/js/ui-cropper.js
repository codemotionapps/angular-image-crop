...
function uiCropper($timeout, CropHost, CropPubSub){
	return {
		...
		link(scope, element){
			...
			// Init Crop Host
			const cropHost = new CropHost(element.find(`canvas`), {}, events);
			...
			const updateAreaCoords = function(scope){
				scope.areaCoords = cropHost.getAreaCoords();
			};

			const updateResultImage = function(scope, force, callback){
				if(scope.image !== `` && (!scope.liveView.block || force)){
					const resultImageObj = cropHost.getResultImage();
					let resultImage;
			...
			const updateCropject = function(scope){
				const areaCoords = cropHost.getAreaCoords();
