...
function cropArea(CropCanvas){
	...
	CropArea.prototype._preventBoundaryCollision = function(size){
		...
		let se = this._southEastBound(size);
		...
		let newSizeWidth = this._forceAspectRatio ? size.w : se.x - nw.x;
		let newSizeHeight = this._forceAspectRatio ? size.h : se.y - nw.y;
	...
	CropArea.prototype._dontDragOutside = function(){
		...
		if(this._x < halfWidth){
			this._x = halfWidth;
		}
		if(this._x > w - halfWidth){
			this._x = w - halfWidth;
		}

		const halfHeight = this._height / 2;
		if(this._y < halfHeight){
			this._y = halfHeight;
		}
		if(this._y > h - halfHeight){
			this._y = h - halfHeight;
		}
	};
